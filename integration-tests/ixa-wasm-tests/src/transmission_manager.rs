...
fn attempt_infection(context: &mut Context) {
    ...
    let person_to_infect: PersonId = context
        .sample_entity(TransmissionRng, Person)
        .expect("Wasm infection model requires a nonempty population");
    ...
    let infection_distribution =
        Exp::new(FOI).expect("Wasm infection model requires a positive force of infection");
