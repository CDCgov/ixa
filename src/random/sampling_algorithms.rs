...
pub fn sample_single_from_known_length<'a, Container: HasLen + HasIter, R: Rng>(
    rng: &mut R,
    set: &'a Container,
) -> Option<Container::Item<'a>> {
    let len = set.len();
    if len == 0 {
        return None;
    }
...
pub fn sample_single_l_reservoir<'a, Container: HasIter, R: Rng>(
    rng: &mut R,
    set: &'a Container,
) -> Option<Container::Item<'a>> {
    let mut chosen_item: Option<Container::Item<'a>> = None; // the currently selected element
...
pub fn sample_multiple_from_known_length<'a, Container: HasLen + HasIter, R: Rng>(
    rng: &mut R,
    set: &'a Container,
    requested: usize,
) -> Vec<Container::Item<'a>> {
    ...
    for (idx, person_id) in set.iter().enumerate() {
        if idx == next_idx {
...
pub fn sample_multiple_l_reservoir<'a, Container: HasLen + HasIter, R: Rng>(
    rng: &mut R,
    set: &'a Container,
    requested: usize,
) -> Vec<Container::Item<'a>> {
