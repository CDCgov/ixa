        ...
        if let Some(multi_property_id) = query.multi_property_type_id() {
                ...
                .get_mut(&multi_property_id)
            {
                ...
                if let Some(people_set) = index.get_with_hash(query.multi_property_value_hash()) {
                    ...
                    // The set is not randomly indexable, so we have to scan through the entries.
        ...
        R::RngType: Rng,
    {
        ...
        if let Some(multi_property_id) = query.multi_property_type_id() {
                ...
                .get_mut(&multi_property_id)
            {
                ...
                if let Some(people_set) = index.get_with_hash(query.multi_property_value_hash()) {
                    // This little trick with `u32` makes this branch 30% faster.
                    let result = self.sample(rng_id, |rng| {
                        rng.gen_range(0..people_set.len() as u32) as usize
                    });
