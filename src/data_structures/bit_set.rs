...
impl BitSet {
    ...
    /// Use `get_unchecked` in the hot path if bounds are guaranteed.
    #[inline(always)]
    ...
    ///
    /// # Safety
    /// Use this if you can guarantee `n` is within bounds.
    #[inline(always)]
    pub unsafe fn get_unchecked(&self, n: usize) -> bool {
        match &self.storage {
            // SAFETY: n < 64, so the shift is well defined.
            Storage::Inline(word) => (word >> n) & 1 != 0,
            // SAFETY: n is a valid property ID < num_bits, so n >> 6 < words.len()
            Storage::Heap(words) => (words.get_unchecked(n >> 6) >> (n & 63)) & 1 != 0,
        }
    }
...
mod tests {
    ...
    #[test]
    fn get_unchecked_reads_inline_and_heap_bits() {
        let mut inline = BitSet::new(64);
        inline.set(63);
        let mut heap = BitSet::new(65);
        heap.set(64);

        // SAFETY: All queried indices are within the respective BitSet capacities.
        unsafe {
            assert!(!inline.get_unchecked(0));
            assert!(inline.get_unchecked(63));
            assert!(!heap.get_unchecked(63));
            assert!(heap.get_unchecked(64));
        }
    }
}
