...
A [`PropertyList<E>`] is a collection of distinct properties of the same [`Entity`] `E`. It
...
for `PropertyEntityValues` types with property types implementing `Property<E>`. Using properties
...
// PropertyEntityValues0 is an empty `PropertyList<E>` for every `E: Entity`.
impl<E: Entity> PropertyList<E> for PropertyEntityValues0<E> {
    fn validate() -> Result<(), String> {
        Ok(())
    }
    fn contains_properties(property_type_ids: &[TypeId]) -> bool {
...
// PropertyEntityValues1 is a `PropertyList` of length 1
impl<E: Entity, P0: Property<E>> PropertyList<E> for PropertyEntityValues1<E, P0> {
    fn validate() -> Result<(), String> {
        Ok(())
    }
    fn contains_properties(property_type_ids: &[TypeId]) -> bool {
        property_type_ids.is_empty()
            || property_type_ids.len() == 1 && property_type_ids[0] == P0::type_id()
    }
    fn set_values_for_entity(&self, entity_id: EntityId<E>, property_store: &PropertyStore<E>) {
        let property_value_store = property_store.get::<P0>();
        property_value_store.set(entity_id, self._0);
    }
}

// Used only within this module.
macro_rules! impl_property_list {
    ($struct_name:ident, $ct:literal) => {
        seq!(N in 0..$ct {
            impl<E: Entity, #( P~N: Property<E>,)*> PropertyList<E> for $struct_name<E, #(P~N,)*> {
                fn validate() -> Result<(), String> {
                ...
                fn set_values_for_entity(&self, entity_id: EntityId<E>, property_store: &PropertyStore<E>){
                    #({
                        ...
                        property_value_store.set(entity_id, self._~N);
                    })*
                }
            }
        });
...
// Generate impls for PropertyEntityValues2 through PropertyEntityValues10.
impl_property_list!(PropertyEntityValues2, 2);
impl_property_list!(PropertyEntityValues3, 3);
impl_property_list!(PropertyEntityValues4, 4);
impl_property_list!(PropertyEntityValues5, 5);
impl_property_list!(PropertyEntityValues6, 6);
impl_property_list!(PropertyEntityValues7, 7);
impl_property_list!(PropertyEntityValues8, 8);
impl_property_list!(PropertyEntityValues9, 9);
impl_property_list!(PropertyEntityValues10, 10);
