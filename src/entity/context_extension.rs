...
pub trait ContextEntitiesExt {
    ...
    ///
    /// Supplying `Person` or `with!(Person)` is equivalent to calling
    /// `get_entity_count::<E>()`.
    ...
    ///
    /// To sample from the entire population, pass in `Person` or `with!(Person)`.
...
impl ContextEntitiesExt for Context {
    ...
    fn with_query_results<'a, E: Entity, Q: Query<E>>(
        ...
        if query.is_empty_query() {
            warn!("Called Context::with_query_results() with a whole-population query. Prefer Context::get_entity_iterator::<E>() for working with the entire population.");
...
mod tests {
    ...
    fn query_results_respect_index_modes() {
        ...
        for mode in modes {
            ...
            assert_eq!(
                context.query_entity_count(with!(Person, existing_value,)),
                2
            );
