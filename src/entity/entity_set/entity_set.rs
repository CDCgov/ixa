...
impl<'a, E: Entity> EntitySet<'a, E> {
    ...
    pub(crate) fn from_intersection_sources(mut sources: Vec<SourceSet<'a, E>>) -> Self {
        ...
        sources.sort_unstable_by_key(SourceSet::len_upper);
    ...
    pub fn union(self, other: Self) -> Self {
        ...
        let (left, right) = if self.len_upper() >= other.len_upper() {
            (self, other)
        } else {
            (other, self)
        };
    ...
    pub fn intersection(self, other: Self) -> Self {
        ...
        // Keep intersections sorted smallest-to-largest so iterators can take the
        // first source as the driver and membership checks short-circuit quickly.
        sets.sort_unstable_by_key(|a| a.len_upper());
        EntitySet(EntitySetInner::Intersection(sets))
    }
    ...
    /// Upper bound on the number of entities in this set.
    pub fn len_upper(&self) -> usize {
        match self {
            EntitySet(EntitySetInner::Source(source)) => source.len_upper(),
            EntitySet(EntitySetInner::Union(a, b)) => a.len_upper().saturating_add(b.len_upper()),
            EntitySet(EntitySetInner::Intersection(sets)) => {
                sets.iter().map(EntitySet::len_upper).min().unwrap_or(0)
            }
            EntitySet(EntitySetInner::Difference(a, _)) => a.len_upper(),
        }
    }
...
mod tests {
    ...
    fn from_source_entity_and_population() {
        ...
        assert_eq!(population.len_upper(), 3);
    }
    ...
    fn len_upper_rules() {
        ...
        assert_eq!(union.len_upper(), a.borrow().len() + b.borrow().len());

        let intersection = as_entity_set(&a).intersection(as_entity_set(&b));
        assert_eq!(
            intersection.len_upper(),
            a.borrow().len().min(b.borrow().len())
        );

        let difference = as_entity_set(&a).difference(as_entity_set(&b));
        assert_eq!(difference.len_upper(), a.borrow().len());
    }
