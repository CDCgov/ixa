...
//! execution.
//!
//! `SourceSet` is the membership-oriented view (`contains`, `len_upper`), while
...
impl<'a, E: Entity> SourceSet<'a, E> {
    ...
    pub(super) fn len_upper(&self) -> usize {
        match self {
            SourceSet::Empty => 0,
            SourceSet::Population(population) => *population,
            SourceSet::Entity(_) => 1,
...
mod tests {
    ...
    fn source_set_variants_basic_behavior() {
        ...
        assert_eq!(singleton.len_upper(), 1);
