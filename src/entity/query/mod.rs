...
///
/// This is not meant to be used directly, but rather as a backing for the q! macro/
...
mod tests {
    ...
    fn all_macro_no_properties() {
        ...
        // q!(Person) should match all Person entities
        let query = q!(Person);
        assert_eq!(context.query_entity_count(query), 2);
    }
    ...
    fn all_macro_single_property() {
        ...
        // q!(Person, Age(42)) should match entities with Age = 42
        let query = q!(Person, Age(42));
        assert_eq!(context.query_entity_count(query), 2);
    }
    ...
    fn all_macro_multiple_properties() {
        ...
        // q!(Person, Age(42), RiskCategory::High) should match one entity
        let query = q!(Person, Age(42), RiskCategory::High);
        assert_eq!(context.query_entity_count(query), 1);
    ...
    fn all_macro_as_property_list_for_add_entity() {
        ...
        // Use q! macro result to add an entity
        let props = q!(Person, Age(42), RiskCategory::High);
