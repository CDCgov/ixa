...
fn schedule_birth(context: &mut Context) {
    let parameters = context
        ...
        .unwrap()
        .clone();
    let person = context.add_entity(q!(Person, Age(0))).unwrap();
...
fn schedule_death(context: &mut Context) {
    ...
    if let Some(person) = context.sample_entity(PeopleRng, q!(Person, Alive(true))) {
...
pub fn init(context: &mut Context) {
    ...
    for _ in 0..parameters.population {
        ...
        let person = context.add_entity(q!(Person, Age(age))).unwrap();
...
mod test {
    ...
    fn test_birth_death() {
        ...
        let person1 = context.add_entity(q!(Person, Age(10))).unwrap();
        ...
        context.add_plan(380.0, move |context| {
            *person2_clone.borrow_mut() = Some(context.add_entity(q!(Person, Age(0))).unwrap());
        });
        context.add_plan(400.0, move |context| {
            context.set_property(person1, Alive(false));
        });
        context.add_plan(390.0, |context| {
            let pop = context.query_entity_count(q!(Person, Alive(true)));
            assert_eq!(pop, 2);
        });
    ...
    fn test_null_death_rates() {
        ...
        context
            ...
            .unwrap();
        context.init_random(p_values.seed);
        let _person = context.add_entity(q!(Person, Age(0))).unwrap();
    ...
    fn test_sample_person_group() {
        ...
        for age in &age_vec {
            people.push(context.add_entity(q!(Person, Age(*age))).unwrap());
        }
