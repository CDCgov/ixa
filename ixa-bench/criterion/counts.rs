...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("single_property_unindexed_entities", |bencher| {
        bencher.iter(|| {
            black_box(context.query_entity_count(black_box(all!(Person, EHomeId(HOME_VAL)))));
        });
    });
    ...
    group.bench_function("multi_property_unindexed_entities", |bencher| {
        bencher.iter(|| {
            black_box(context.query_entity_count(black_box(all!(Person, EAge(30), ESchoolId(1)))));
        });
    });
    ...
    group.bench_function("index_after_adding_entities", |bencher| {
        bencher.iter_with_setup(
            ...
            |mut ctx| {
                ...
                black_box(ctx.query_entity_count(black_box(all!(Person, EHomeId(HOME_VAL)))));
            },
        );
    });
