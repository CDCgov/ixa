...
trait InfectionLoop: ContextEntitiesExt {
...
impl InfectionLoop for Context {
    ...
    fn random_contact(&mut self, source: PersonId) -> Option<PersonId> {
        ...
        Some(self.get_data(HouseholdPlugin).members.get(household)?[idx])
    }
    fn infect_person(&mut self, p: PersonId, t: Option<f64>) {
    ...
    fn setup(&mut self) {
        ...
        // Seed infections
        let sampled_entities: Vec<PersonId> = self.sample_entities::<Person, _, _>(
            NextPersonRng,
...
mod test {
    ...
    fn infected_counts() {
        ...
        let p = model
            .ctx
            .sample_entity(NextPersonRng, with!(Person, InfectionStatus::Susceptible))
            .unwrap();
